...
app.post('/api/reports', async (req, res) => {
  try {
    ...
    if (emails.length) { emailResult = await email.sendReport(report, crew, emails); }
    res.json({ success: true, reportId, emailSent: emailResult ? emailResult.success : false });
  } catch (err) { console.error('Submit error:', err); res.status(500).json({ error: err.message }); }
});

// Resend: send a single report by ID
...
// Resend: send all reports for a date
app.post('/api/send-day/:date', async (req, res) => {
  try {
    const reports = q.reportsByDate.all(req.params.date);
    if (!reports.length) return res.json({ success: false, error: 'No reports for this date' });
    const emails = q.activeRecipients();
    if (!emails.length) return res.json({ success: false, error: 'No active recipients' });
    let sent = 0;
    for (const r of reports) {
      const crew = q.reportCrew.all(r.id);
      await email.sendReport(r, crew, emails);
      sent++;
    }
