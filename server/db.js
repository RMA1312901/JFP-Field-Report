...
try { db.exec("CREATE TABLE IF NOT EXISTS mep (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE)"); } catch(e) {}
...
const submitReport = db.transaction((d) => {
  ...
  if (d.sessionData) { const key = d.sessionKey || d.foreman; q.saveSession.run(key, JSON.stringify(d.sessionData)); }
  return rid;
});
