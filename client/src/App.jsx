        ...
        <div style={{ padding:"16px 20px 0" }}>
          {isWorker ? (
            ...
            <>
              ...
              {crew.map((c, i) => (
                <div key={i} style={{ background:C.card, border:`1px solid ${(c.is_foreman || c.isForeman) ? `${C.green}44` : C.border}`, borderRadius:12, padding:12, marginBottom:8 }}>
          ...
          <button onClick={() => {
            let txt;
            if (isWorker) {
              const jLines = jobs.map(j => `  Job #${j.jobNumber}: ${j.hours}h — ${j.description}${j.equipment ? ` (Equip: ${j.equipment})` : ""}`).join("\n");
              txt = `NON-FOREMAN REPORT\nWorker: ${report.foreman}\nDate: ${fmtDate(report.date)}\n\nJOBS:\n${jLines}\n\nTotal: ${jobs.reduce((s,j)=>s+(parseFloat(j.hours)||0),0)}h`;
            } else {
              const cl = crew.map(c=>`  ${c.name}${(c.is_foreman||c.isForeman)?" (FM)":""}: ${c.hours}h${c.note?` — ${c.note}`:""}`).join("\n");
              const el = eqList.length ? "\nEQUIP:\n"+eqList.map(eq=>`  ${eq.name}: ${eq.hours}h`).join("\n") : "";
              txt = `FOREMAN REPORT\nForeman: ${report.foreman}\nDate: ${fmtDate(report.date)}\nSite: ${report.site}\nSafety: ${safetyOk?"Yes":"No"}\nSubs: ${subsOk?subs.join(", ")||"Yes":"No"}${el}\n\nWORK:\n${desc}\n\nCREW (${crew.length}, ${hrs}h):\n${cl}`;
            }
            const subj = encodeURIComponent(`Field Report — ${report.foreman} — ${fmtDate(report.date)}`);
...
export default function App() {
  ...
  const goHome = () => {
    ...
    setStep("login"); setErrors({}); setForeman(""); setCrew([]); setSite(""); setWorkDesc(""); setJobs([{ id: uid(), jobNumber: "", hours: "", description: "", equipment: "" }]); setReportType("foreman"); setShowAllForemen(false); setHasEquipment(false); setEquipmentList([]); setDate(todayStr()); setOvertimeConfirmed(false); setDupeWarning(null);
  };
  ...
  const checkOvertimeAndProceed = (callback) => {
    ...
    if (reportType === "foreman") {
      ...
      const total = jobs.filter(j => j.jobNumber.trim()).reduce((s, j) => s + (parseFloat(j.hours) || 0), 0);
      if (total > 8) overHrs = total;
    }
    if (overHrs) {
  ...
  const [dupeSubmitFn, setDupeSubmitFn] = useState(null);

  const checkDupeAndSubmit = async (submitFn) => {
    try {
      const { reports } = await api.getReportsByDate(date);
      const existing = reports.find(r => r.foreman === foreman);
      if (existing) {
        setDupeWarning(date);
        setDupeSubmitFn(() => submitFn);
      } else {
        submitFn();
      }
    } catch { submitFn(); }
  ...
  const submitAnotherDay = () => {
    setErrors({});
    // Advance date by 1 day, keep hours/crew/site
    ...
    else { setJobs(jobs.map(j => ({ ...j, id: uid(), description: "" }))); setStep("workerForm"); }
  };
  ...
  // ── View report helper ──
  const viewReport = async (rpt) => {
    if (rpt && rpt.id && !rpt._loaded) {
      try { const { report } = await api.getReportById(rpt.id); setViewingReport({ ...report, _loaded: true }); }
      catch (e) { console.error(e); setViewingReport(rpt); }
    } else { setViewingReport(rpt); }
  };
  ...
  const handleToggleOff = async (name, date) => {
    try {
      await api.toggleOff(name, date);
      await loadMonth(calYM);
    } catch (e) { console.error(e); }
  };
  ...
  // ═══════════ LOGIN ═══════════
  if (step === "login") {
    const allOthers = [...allForemen.map(f => ({ name: f, type: "foreman" })), ...allWorkers.map(w => ({ name: w, type: "worker" }))];
    const others = savedUser ? allOthers.filter(p => p.name !== savedUser.name) : allOthers;
    return (
      <div style={{ ...wrap, position: "relative" }}>
        ...
        <div style={{ padding: "24px 20px 0" }}>
          {savedUser ? (<>
            <button onClick={() => selectUser(savedUser.name, savedUser.type)} style={{ width: "100%", padding: "16px 18px", fontSize: 18, fontWeight: 700, background: T.card, border: `2px solid #2563eb44`, borderRadius: 12, color: T.text, cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span>{savedUser.name}</span><span style={{ color: "#2563eb", fontSize: 18 }}>→</span></button>
            {!showAllForemen && others.length > 0 && (
            ...
            {showAllForemen && (<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {others.map(p => (
                <button key={p.name} onClick={() => { selectUser(p.name, p.type); setShowAllForemen(false); }} style={{ width: "100%", padding: "13px 18px", fontSize: 16, fontWeight: 600, background: T.card, border: `1.5px solid ${T.border}`, borderRadius: 12, color: T.text, cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span>{p.name}</span><span style={{ color: T.dim, fontSize: 16 }}>→</span></button>))}
              <button onClick={() => setAddModal({ type: "_custom", title: "Custom Worker", placeholder: "Name" })} style={{ padding: "13px 18px", fontSize: 14, fontWeight: 600, background: T.input, border: `1.5px dashed ${T.border}`, borderRadius: 12, color: T.accent, cursor: "pointer", textAlign: "left" }}>+ Custom</button>
            </div>)}
          </>) : (<>
            ...
            <div style={{ padding: "24px 0 0" }}><div style={divider} /><div style={{ paddingTop: 18 }}>
  ...
  if (step === "workerForm") {
    ...
    return (
      <div style={wrap}>
        ...
        {dupeWarning && (<DupeDateWarning date={fmtDate(dupeWarning)} onContinue={() => { setDupeWarning(null); if (dupeSubmitFn) dupeSubmitFn(); setDupeSubmitFn(null); }} onChangeDate={() => { setDupeWarning(null); setDupeSubmitFn(null); }} C={T} />)}
      </div>
    );
  }
      ...
      <div style={wrap}>
        ...
        <div style={sec}>
          ...
          <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
            <button style={btnMain(sending)} onClick={() => checkDupeAndSubmit(handleWorkerSubmit)} disabled={sending}>{sending ? "Sending..." : "Submit Report"}</button>
    ...
    <div style={wrap}>
      ...
      <div style={{ padding: "40px 20px", textAlign: "center" }}>
        ...
        <button style={btnGhost} onClick={() => {
          if (reportType === "foreman") { setCrew(crew.map(c => ({ ...c, hours: "", note: "" }))); setWorkDesc(""); setStep("form"); setErrors({}); }
  ...
  if (step === "admin") {
    ...
    const allPeople = [...allForemen.map(f => ({ name: f, type: "foreman" })), ...allWorkers.map(w => ({ name: w, type: "worker" }))];
    const dayStatuses = calSelectedDay ? allPeople.map(p => {
      const rpt = dayReports.find(r => r.foreman === p.name);
      return { name: p.name, type: p.type, submitted: !!rpt, report: rpt || null };
    }) : [];
        ...
        <div style={sec}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2, textAlign: "center" }}>
            ...
            {Array.from({ length: days }).map((_, i) => {
              ...
              const allDone = has && allForemen.every(f => dayRpts.some(r => r.foreman === f) || dayOff.some(o => o.name === f));
        ...
        {calSelectedDay && (<>
          ...
          <div style={sec}>
            ...
            {dayStatuses.map(fs => {
              ...
              const isOff = monthOffDays.some(o => o.name === fs.name && o.date === ds);
              const isWorkerRpt = fs.report && (fs.report.report_type || fs.report.reportType) === "worker";
              const jobsData = fs.report ? (fs.report.jobs_data ? (typeof fs.report.jobs_data === 'string' ? JSON.parse(fs.report.jobs_data) : fs.report.jobs_data) : (fs.report.jobs || [])) : [];
              const detail = fs.submitted ? (isWorkerRpt
                ? `${jobsData.length} jobs · ${jobsData.reduce((s, j) => s + (parseFloat(j.hours) || 0), 0)}h`
                : `${fs.report.site} · ${(fs.report.total_hrs || fs.report.totalHrs || 0)}h`
              ) : null;
              const borderColor = fs.submitted ? `${T.green}44` : isOff ? `${T.danger}44` : T.border;
              return (
                <div key={fs.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: T.card, border: `1px solid ${borderColor}`, borderRadius: 10, padding: "10px 14px", marginBottom: 6 }}>
                  <div onClick={() => fs.report && viewReport(fs.report)} style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, cursor: fs.submitted ? "pointer" : "default" }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: fs.submitted ? T.green : isOff ? T.danger : `${T.dim}44` }} />
                    <span style={{ fontSize: 15, fontWeight: 600, color: fs.submitted ? T.green : isOff ? T.danger : T.dim }}>{fs.name}</span>
                  </div>
                  {fs.submitted ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span onClick={() => viewReport(fs.report)} style={{ color: T.accent, fontSize: 14, cursor: "pointer" }}>→</span>
                    </div>
                  ) : isOff ? (
                    <button onClick={() => handleToggleOff(fs.name, ds)} style={{ background: `${T.danger}15`, border: `1px solid ${T.danger}33`, borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700, color: T.danger, cursor: "pointer" }}>OFF ✕</button>
                  ) : (
                    <button onClick={() => handleToggleOff(fs.name, ds)} style={{ background: `${T.danger}10`, border: `1px solid ${T.border}`, borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, color: T.dim, cursor: "pointer" }}>Off</button>
                  )}
                </div>
              );
            })}
  ...
  if (step === "review") return (
    <div style={wrap}>
      ...
      <div style={sec}>
        ...
        <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 10 }}>
          <button style={btnMain(sending)} onClick={() => checkDupeAndSubmit(handleSubmit)} disabled={sending}>{sending ? "Sending..." : "Submit Report"}</button>
